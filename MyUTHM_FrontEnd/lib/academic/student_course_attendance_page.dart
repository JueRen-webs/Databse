...
class StudentCourseAttendancePage extends StatelessWidget {
  ...
  bool get _hasWarningLetter => records.isNotEmpty && _attendancePercent < 80;

  @override
  Widget build(BuildContext context) {
    if (courseData != null) {
      return FutureBuilder<List<Map<String, dynamic>>>(
        future: DatabaseHelper.instance.getStudentAttendanceForSection(
          sectionId: courseData!['section_id'] ?? '',
        ...
        builder: (context, snapshot) {
          ...
          final resolvedRecords = rows.map(_recordFromRow).toList();
          final present = resolvedRecords.where((r) => r.isPresent).length;
          final percent = resolvedRecords.isEmpty
              ? 0
              : ((present / resolvedRecords.length) * 100).round();

          return StudentCourseAttendancePage(
            showChrome: showChrome,
            courseName: courseData!['name'] ?? 'Course',
            lecturerName: courseData!['lecturer'] ?? 'Lecturer',
  ...
  StudentCourseAttendanceRecord _recordFromRow(Map<String, dynamic> row) {
    final rawDate = row['Session_Date']?.toString() ?? '';
    var displayDate = rawDate;
    try {
      displayDate =
          DateFormat('EEEE, d MMM yyyy').format(DateTime.parse(rawDate));
    } catch (_) {}

    return StudentCourseAttendanceRecord(
      date: displayDate,
      startTime: _formatTime(row['Start_Time']?.toString() ?? ''),
      endTime: _formatTime(row['End_Time']?.toString() ?? ''),
      isPresent: row['Attendance_Status']?.toString() == 'Present',
    );
  }
